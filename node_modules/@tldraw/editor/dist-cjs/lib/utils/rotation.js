...
function applyRotationToSnapshotShapes({
  ...
  editor.updateShapes(
    shapeSnapshots.map(({ shape, initialPagePoint }) => {
      const parentTransform = (0, import_tlschema.isShapeId)(shape.parentId) ? editor.getPageTransformById(shape.parentId) : import_primitives.Matrix2d.Identity();
      const newPagePoint = import_primitives.Vec2d.RotWith(initialPagePoint, selectionPageCenter, delta);
      const newLocalPoint = import_primitives.Matrix2d.applyToPoint(
        // use the current parent transform in case it has moved/resized since the start
        // (e.g. if rotating a shape at the edge of a group)
        import_primitives.Matrix2d.Inverse(parentTransform),
        newPagePoint
      );
      const newRotation = (0, import_primitives.canolicalizeRotation)(shape.rotation + delta);
      return {
        id: shape.id,
        type: shape.type,
        x: newLocalPoint.x,
