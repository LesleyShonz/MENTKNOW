...
function getArrowPoints(info, side, strokeWidth) {
  ...
  const compareLength = info.isStraight ? import_primitives.Vec2d.Dist(PB, PT) : Math.abs(info.bodyArc.length);
  const length = Math.max(Math.min(compareLength / 5, strokeWidth * 3), strokeWidth);
  let P0;
  if (info.isStraight) {
    P0 = import_primitives.Vec2d.Nudge(PT, PB, length);
  } else {
    const ints = (0, import_primitives.intersectCircleCircle)(PT, length, info.handleArc.center, info.handleArc.radius);
...
function getArrowhead({ point, int }) {
  const PL = import_primitives.Vec2d.RotWith(int, point, import_primitives.PI / 6);
  const PR = import_primitives.Vec2d.RotWith(int, point, -import_primitives.PI / 6);
...
function getInvertedTriangleHead({ point, int }) {
  const d = import_primitives.Vec2d.Sub(int, point).div(2);
  const PL = import_primitives.Vec2d.Add(point, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
  const PR = import_primitives.Vec2d.Sub(point, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
  return `M ${PL.x} ${PL.y} L ${int.x} ${int.y} L ${PR.x} ${PR.y} Z`;
}
function getDotHead({ point, int }) {
  const A = import_primitives.Vec2d.Lrp(point, int, 0.45);
  const r = import_primitives.Vec2d.Dist(A, point);
  return `M ${A.x - r},${A.y}
  a ${r},${r} 0 1,0 ${r * 2},0
  a ${r},${r} 0 1,0 -${r * 2},0 `;
}
function getDiamondHead({ point, int }) {
  const PB = import_primitives.Vec2d.Lrp(point, int, 0.75);
  const PL = import_primitives.Vec2d.RotWith(PB, point, import_primitives.PI / 4);
  const PR = import_primitives.Vec2d.RotWith(PB, point, -import_primitives.PI / 4);
  const PQ = import_primitives.Vec2d.Lrp(PL, PR, 0.5);
  PQ.add(import_primitives.Vec2d.Sub(PQ, point));
  return `M ${PQ.x} ${PQ.y} L ${PL.x} ${PL.y} ${point.x} ${point.y} L ${PR.x} ${PR.y} Z`;
}
function getSquareHead({ int, point }) {
  const PB = import_primitives.Vec2d.Lrp(point, int, 0.85);
  const d = import_primitives.Vec2d.Sub(PB, point).div(2);
  const PL1 = import_primitives.Vec2d.Add(point, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
  const PR1 = import_primitives.Vec2d.Sub(point, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
  const PL2 = import_primitives.Vec2d.Add(PB, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
  const PR2 = import_primitives.Vec2d.Sub(PB, import_primitives.Vec2d.Rot(d, import_primitives.TAU));
