...
function getStraightArrowInfo(editor, shape) {
  ...
  const c = import_primitives.Vec2d.Med(a, b);
  const uAB = import_primitives.Vec2d.Sub(b, a).uni();
  ...
  if (startShapeInfo && endShapeInfo && !isSelfIntersection && !startShapeInfo.isExact && !endShapeInfo.isExact) {
    if (endShapeInfo.didIntersect && !startShapeInfo.didIntersect) {
      if (startShapeInfo.util.isClosed(startShapeInfo.shape)) {
        a.setTo(import_primitives.Vec2d.Nudge(b, a, minDist));
      }
    } else if (!endShapeInfo.didIntersect) {
      if (endShapeInfo.util.isClosed(endShapeInfo.shape)) {
        b.setTo(import_primitives.Vec2d.Nudge(a, b, minDist));
      }
    }
  }
  const u = import_primitives.Vec2d.Sub(b, a).uni();
  const didFlip = !import_primitives.Vec2d.Equals(u, uAB);
  if (!isSelfIntersection) {
    if (startShapeInfo && arrowheadStart !== "none" && !startShapeInfo.isExact) {
      const offset = import_shared.BOUND_ARROW_OFFSET + import_default_shape_constants.STROKE_SIZES[shape.props.size] / 2 + ("size" in startShapeInfo.shape.props ? import_default_shape_constants.STROKE_SIZES[startShapeInfo.shape.props.size] / 2 : 0);
      minDist -= offset;
      a.nudge(b, offset * (didFlip ? -1 : 1));
    }
    if (endShapeInfo && arrowheadEnd !== "none" && !endShapeInfo.isExact) {
      const offset = import_shared.BOUND_ARROW_OFFSET + import_default_shape_constants.STROKE_SIZES[shape.props.size] / 2 + ("size" in endShapeInfo.shape.props ? import_default_shape_constants.STROKE_SIZES[endShapeInfo.shape.props.size] / 2 : 0);
      minDist -= offset;
  ...
  if (startShapeInfo && endShapeInfo) {
    if (didFlip) {
      b.setTo(import_primitives.Vec2d.Add(a, u.mul(-minDist)));
    } else if (import_primitives.Vec2d.Dist(a, b) < import_shared.MIN_ARROW_LENGTH / 2) {
      b.setTo(import_primitives.Vec2d.Add(a, u.mul(import_shared.MIN_ARROW_LENGTH / 2)));
    }
  }
  if (didFlip) {
    c.setTo(import_primitives.Vec2d.Med(terminalsInArrowSpace.start, terminalsInArrowSpace.end));
  } else {
    c.setTo(import_primitives.Vec2d.Med(a, b));
  }
  const length = import_primitives.Vec2d.Dist(a, b);
  return {
    isStraight: true,
    start: {
...
function updateArrowheadPointWithBoundShape(point, opposite, arrowPageTransform, targetShapeInfo) {
  ...
  const pageFrom = import_primitives.Matrix2d.applyToPoint(arrowPageTransform, opposite);
  const pageTo = import_primitives.Matrix2d.applyToPoint(arrowPageTransform, point);
  const targetFrom = import_primitives.Matrix2d.applyToPoint(import_primitives.Matrix2d.Inverse(targetShapeInfo.transform), pageFrom);
  const targetTo = import_primitives.Matrix2d.applyToPoint(import_primitives.Matrix2d.Inverse(targetShapeInfo.transform), pageTo);
  const isClosed = targetShapeInfo.util.isClosed(targetShapeInfo.shape);
  const fn = isClosed ? import_primitives.intersectLineSegmentPolygon : import_primitives.intersectLineSegmentPolyline;
  const intersection = fn(targetFrom, targetTo, targetShapeInfo.util.outline(targetShapeInfo.shape));
  let targetInt;
  if (intersection !== null) {
    targetInt = intersection.sort((p1, p2) => import_primitives.Vec2d.Dist(p1, targetFrom) - import_primitives.Vec2d.Dist(p2, targetFrom))[0] ?? (isClosed ? void 0 : targetTo);
  }
  if (targetInt === void 0) {
    return;
  }
  const pageInt = import_primitives.Matrix2d.applyToPoint(targetShapeInfo.transform, targetInt);
  const arrowInt = import_primitives.Matrix2d.applyToPoint(import_primitives.Matrix2d.Inverse(arrowPageTransform), pageInt);
