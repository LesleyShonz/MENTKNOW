...
function useExportAs() {
  ...
  return (0, import_react.useCallback)(
    async function exportAs(ids = editor.selectedIds, format = "png") {
      ...
      switch (format) {
        case "svg": {
          const dataURL = await (0, import_editor.getSvgAsDataUrl)(svg);
          (0, import_editor.downloadDataURLAsFile)(dataURL, `${name || "shapes"}.svg`);
          return;
        }
        case "webp":
        case "png": {
          const image = await (0, import_editor.getSvgAsImage)(svg, {
            type: format,
            quality: 1,
            scale: 2
          });
          ...
          (0, import_editor.downloadDataURLAsFile)(dataURL, `${name || "shapes"}.${format}`);
        ...
        case "json": {
          ...
          (0, import_editor.downloadDataURLAsFile)(dataURL, `${name || "shapes"}.json`);
          URL.revokeObjectURL(dataURL);
          return;
        }
        default:
