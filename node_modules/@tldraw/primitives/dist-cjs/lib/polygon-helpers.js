...
function precise(A) {
  return `${(0, import_utils.toDomPrecision)(A.x)},${(0, import_utils.toDomPrecision)(A.y)} `;
}
function rng(seed = "") {
  let x = 0;
  let y = 0;
...
function getRoundedPolygonPoints(id, outline, offset, roundness, passes) {
  ...
  for (let i = 0, n = len * passes; i < n; i++) {
    p1 = import_Vec2d.Vec2d.AddXY(outline[(i + 1) % len], random() * offset, random() * offset);
    const delta = import_Vec2d.Vec2d.Sub(p1, p0);
    const distance = import_Vec2d.Vec2d.Len(delta);
    const vector = import_Vec2d.Vec2d.Div(delta, distance).mul(Math.min(distance / 4, roundness));
    results.push(import_Vec2d.Vec2d.Add(p0, vector), import_Vec2d.Vec2d.Add(p1, vector.neg()), p1);
    p0 = p1;
  }
  return results;
}
function getDrawLinePathData(id, outline, strokeWidth) {
  ...
  for (let i = 0, n = len - 1; i < n; i++) {
    p1 = import_Vec2d.Vec2d.AddXY(outline[i + 1], random() * offset, random() * offset);
    s1 = import_Vec2d.Vec2d.AddXY(outline[i + 1], random(), random() * offset);
    const delta = import_Vec2d.Vec2d.Sub(p1, p0);
    const distance = import_Vec2d.Vec2d.Len(delta);
    const vector = import_Vec2d.Vec2d.Div(delta, distance).mul(Math.min(distance / 4, roundness));
    const q0 = import_Vec2d.Vec2d.Add(p0, vector);
    const q1 = import_Vec2d.Vec2d.Add(p1, vector.neg());
    const sDelta = import_Vec2d.Vec2d.Sub(s1, s0);
    const sDistance = import_Vec2d.Vec2d.Len(sDelta);
    const sVector = import_Vec2d.Vec2d.Div(sDelta, sDistance).mul(Math.min(sDistance / 4, roundness));
    const sq0 = import_Vec2d.Vec2d.Add(s0, sVector);
    const sq1 = import_Vec2d.Vec2d.Add(s1, sVector.neg());
    if (i === n - 1) {
