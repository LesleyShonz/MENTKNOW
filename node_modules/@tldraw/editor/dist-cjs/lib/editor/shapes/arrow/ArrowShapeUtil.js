...
var import_arrowheads = require("./arrow/arrowheads");
var import_curved_arrow = require("./arrow/curved-arrow");
var import_shared = require("./arrow/shared");
var import_straight_arrow = require("./arrow/straight-arrow");
var import_ArrowTextLabel = require("./components/ArrowTextLabel");
let globalRenderIndex = 0;
class ArrowShapeUtil extends import_ShapeUtil.ShapeUtil {
  static type = "arrow";
  canEdit = () => true;
  canBind = () => false;
  isClosed = () => false;
  ...
  getBounds(shape) {
    return import_primitives.Box2d.FromPoints(this.getOutlineWithoutLabel(shape));
  }
  getOutlineWithoutLabel(shape) {
    ...
    if (info.isStraight) {
      if (info.isValid) {
        return [import_primitives.Vec2d.From(info.start.point), import_primitives.Vec2d.From(info.end.point)];
      } else {
        return [new import_primitives.Vec2d(0, 0), new import_primitives.Vec2d(1, 1)];
      }
    }
    if (!info.isValid) {
      return [new import_primitives.Vec2d(0, 0), new import_primitives.Vec2d(1, 1)];
    ...
    const startAngle = import_primitives.Vec2d.Angle(info.bodyArc.center, info.start.point);
    const endAngle = import_primitives.Vec2d.Angle(info.bodyArc.center, info.end.point);
    const a = info.bodyArc.sweepFlag ? endAngle : startAngle;
    const b = info.bodyArc.sweepFlag ? startAngle : endAngle;
    const l = info.bodyArc.largeArcFlag ? -(0, import_primitives.longAngleDist)(a, b) : (0, import_primitives.shortAngleDist)(a, b);
    const r = Math.max(1, info.bodyArc.radius);
    for (let i = 0; i < pointsToPush; i++) {
      const t = i / (pointsToPush - 1);
      const angle = a + l * t;
      const point = (0, import_primitives.getPointOnCircle)(info.bodyArc.center.x, info.bodyArc.center.y, r, angle);
      results[i] = point;
    }
    return results;
  }
  getOutline(shape) {
    ...
    for (let i = 1; i < outlineWithoutLabel.length; i++) {
      ...
      if (!didAddLabel) {
        const nearestIntersectingSideIndex = (0, import_utils.minBy)(
          sideIndexes.filter(
            (sideIndex) => (0, import_primitives.linesIntersect)(sides[sideIndex][0], sides[sideIndex][1], prevPoint, nextPoint)
          ),
          (sideIndex) => import_primitives.Vec2d.DistanceToLineSegment(sides[sideIndex][0], sides[sideIndex][1], prevPoint)
        );
        if (nearestIntersectingSideIndex !== void 0) {
          const intersectingPoint = import_primitives.Vec2d.NearestPointOnLineSegment(
  ...
  get infoCache() {
    return this.editor.store.createComputedCache(
      "arrow infoCache",
      (shape) => {
        return (0, import_shared.getIsArrowStraight)(shape) ? (0, import_straight_arrow.getStraightArrowInfo)(this.editor, shape) : (0, import_curved_arrow.getCurvedArrowInfo)(this.editor, shape);
      }
    );
  }
  getArrowInfo(shape) {
  ...
  onHandleChange = (shape, { handle, isPrecise: isPrecise2 }) => {
    const next = (0, import_utils.deepCopy)(shape);
    switch (handle.id) {
      case "start":
      case "end": {
        const pageTransform = this.editor.getPageTransformById(next.id);
        const pointInPageSpace = import_primitives.Matrix2d.applyToPoint(pageTransform, handle);
        if (this.editor.inputs.ctrlKey) {
          ...
          const target = (0, import_utils.last)(
            this.editor.sortedShapesArray.filter((hitShape) => {
              ...
              if (pageMask) {
                if (!(0, import_primitives.pointInPolygon)(pointInPageSpace, pageMask))
                  return;
              }
              ...
              if (util.isClosed(hitShape)) {
                return (0, import_primitives.pointInPolygon)(pointInTargetSpace, util.outline(hitShape));
              }
          ...
          if (target) {
            ...
            if (precise) {
              precise = import_primitives.Vec2d.Dist(pointInTargetSpace, targetBounds.center) > Math.max(
                4,
      ...
      case "middle": {
        const { start, end } = (0, import_shared.getArrowTerminalsInArrowSpace)(this.editor, next);
        ...
        const point = import_primitives.Vec2d.NearestPointOnLineSegment(A, B, handle, false);
        let bend = import_primitives.Vec2d.Dist(point, med);
        if (import_primitives.Vec2d.Clockwise(point, end, med))
          bend *= -1;
  ...
  onTranslateStart = (shape) => {
    ...
    startBinding = null;
    endBinding = null;
    const { start, end } = (0, import_shared.getArrowTerminalsInArrowSpace)(this.editor, shape);
    return {
      id: shape.id,
      type: shape.type,
      props: {
  ...
  onResize = (shape, info) => {
    ...
    const terminals = (0, import_shared.getArrowTerminalsInArrowSpace)(this.editor, shape);
    const { start, end } = (0, import_utils.deepCopy)(shape.props);
  ...
  component(shape) {
    ...
    const path = info.isStraight ? (0, import_straight_arrow.getSolidStraightArrowPath)(info) : (0, import_curved_arrow.getSolidCurvedArrowPath)(info);
    let handlePath = null;
    if (onlySelectedShape === shape && shouldDisplayHandles) {
      ...
      const { strokeDasharray: strokeDasharray2, strokeDashoffset: strokeDashoffset2 } = (0, import_getPerfectDashProps.getPerfectDashProps)(
        info.isStraight ? import_primitives.Vec2d.Dist(info.start.handle, info.end.handle) : Math.abs(info.handleArc.length),
        sw,
        {
          end: "skip",
          start: "skip",
      ...
      handlePath = shape.props.start.type === "binding" || shape.props.end.type === "binding" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "path",
        {
          ...
          d: info.isStraight ? (0, import_straight_arrow.getStraightArrowHandlePath)(info) : (0, import_curved_arrow.getCurvedArrowHandlePath)(info),
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_SVGContainer.SVGContainer, { id: shape.id, style: { minWidth: 50, minHeight: 50 }, children: [
        includeMask && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("defs", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mask", { id: maskId, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "rect",
            {
              x: (0, import_primitives.toDomPrecision)(-100 + bounds.minX),
              y: (0, import_primitives.toDomPrecision)(-100 + bounds.minY),
              width: (0, import_primitives.toDomPrecision)(bounds.width + 200),
              height: (0, import_primitives.toDomPrecision)(bounds.height + 200),
              fill: "white"
            }
          ),
          labelSize && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "rect",
            {
              x: (0, import_primitives.toDomPrecision)(labelSize.x),
              y: (0, import_primitives.toDomPrecision)(labelSize.y),
              width: (0, import_primitives.toDomPrecision)(labelSize.w),
              height: (0, import_primitives.toDomPrecision)(labelSize.h),
              fill: "black",
              rx: 4,
              ry: 4
            }
            ...
            children: [
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("g", { ...includeMask ? { mask: `url(#${maskId})` } : void 0, children: [
                includeMask && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  "rect",
                  {
                    x: (0, import_primitives.toDomPrecision)(bounds.minX - 100),
                    y: (0, import_primitives.toDomPrecision)(bounds.minY - 100),
                    width: (0, import_primitives.toDomPrecision)(bounds.width + 200),
                    height: (0, import_primitives.toDomPrecision)(bounds.height + 200),
                    opacity: 0
                  }
              ...
              as && maskStartArrowhead && shape.props.fill !== "none" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_ShapeFill.ShapeFill, { d: as, color: shape.props.color, fill: shape.props.fill }),
              ae && maskEndArrowhead && shape.props.fill !== "none" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_ShapeFill.ShapeFill, { d: ae, color: shape.props.color, fill: shape.props.fill }),
              as && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: as }),
              ae && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: ae })
            ]
          }
        ),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: path, className: "tl-hitarea-stroke" })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
  ...
  indicator(shape) {
    ...
    if (!info)
      return null;
    if (import_primitives.Vec2d.Equals(start, end))
      return null;
    const strokeWidth = import_default_shape_constants.STROKE_SIZES[shape.props.size];
    const as = info.start.arrowhead && (0, import_arrowheads.getArrowheadPathForType)(info, "start", strokeWidth);
    const ae = info.end.arrowhead && (0, import_arrowheads.getArrowheadPathForType)(info, "end", strokeWidth);
  ...
  get labelBoundsCache() {
    return this.editor.store.createComputedCache("labelBoundsCache", (shape) => {
      ...
      if (width > 16 * import_default_shape_constants.ARROW_LABEL_FONT_SIZES[size]) {
        width = 16 * import_default_shape_constants.ARROW_LABEL_FONT_SIZES[size];
        const { w: squishedWidth, h: squishedHeight } = this.editor.textMeasure.measureText(text, {
          ...import_default_shape_constants.TEXT_PROPS,
          fontFamily: import_default_shape_constants.FONT_FAMILIES[font],
          fontSize: import_default_shape_constants.ARROW_LABEL_FONT_SIZES[size],
  ...
  getEditingBounds = (shape) => {
    return this.getLabelBounds(shape) ?? new import_primitives.Box2d();
  };
  onEditEnd = (shape) => {
    const {
      id,
  ...
  toSvg(shape, font, colors) {
    ...
    const path = getArrowSvgPath(
      info.isStraight ? (0, import_straight_arrow.getSolidStraightArrowPath)(info) : (0, import_curved_arrow.getSolidCurvedArrowPath)(info),
      color,
      strokeWidth
    );
    const { strokeDasharray, strokeDashoffset } = (0, import_getPerfectDashProps.getPerfectDashProps)(
