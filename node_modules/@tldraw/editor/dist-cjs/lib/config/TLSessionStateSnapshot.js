...
const TAB_ID = window?.[tabIdKey] ?? window?.sessionStorage[tabIdKey] ?? `TLDRAW_INSTANCE_STATE_V1_` + (0, import_data.uniqueId)();
if (window) {
...
const sessionStateSnapshotValidator = import_validate.T.object({
  ...
  pageStates: import_validate.T.arrayOf(
    import_validate.T.object({
      ...
      selectedIds: import_validate.T.arrayOf(import_tlschema.shapeIdValidator),
      focusLayerId: import_tlschema.shapeIdValidator.nullable()
    })
  )
});
...
function createSessionStateSnapshotSignal(store) {
  ...
  return (0, import_signia.computed)("sessionStateSnapshot", () => {
