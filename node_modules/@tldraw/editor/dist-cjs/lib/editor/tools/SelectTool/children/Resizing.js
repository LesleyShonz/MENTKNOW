...
class Resizing extends import_StateNode.StateNode {
  static id = "resizing";
  info = {};
  markId = "";
  ...
  updateShapes() {
    ...
    if (shouldSnap && selectionRotation % import_primitives.TAU === 0) {
      const { nudge } = this.editor.snaps.snapResize({
        dragDelta: import_primitives.Vec2d.Sub(currentPagePoint, originPagePoint),
    ...
    const scaleOriginPage = import_primitives.Vec2d.RotWith(
      altKey ? selectionBounds.center : selectionBounds.getHandlePoint(scaleOriginHandle),
      selectionBounds.point,
      selectionRotation
    );
    const distanceFromScaleOriginNow = import_primitives.Vec2d.Sub(currentPagePoint, scaleOriginPage).rot(
      -selectionRotation
    );
    const distanceFromScaleOriginAtStart = import_primitives.Vec2d.Sub(originPagePoint, scaleOriginPage).rot(
      -selectionRotation
    );
    const scale = import_primitives.Vec2d.DivV(distanceFromScaleOriginNow, distanceFromScaleOriginAtStart);
  ...
  _createSnapshot = () => {
    ...
    const dragHandlePoint = import_primitives.Vec2d.RotWith(
      selectionBounds.getHandlePoint(this.info.handle),
      selectionBounds.point,
      selectionRotation
    );
    const cursorHandleOffset = import_primitives.Vec2d.Sub(originPagePoint, dragHandlePoint);
    ...
    selectedIds.forEach((id) => {
      ...
      if (shape) {
        ...
        if (this.editor.isShapeOfType(shape, import_FrameShapeUtil.FrameShapeUtil) && selectedIds.length === 1)
          return;
        this.editor.visitDescendants(shape.id, (descendantId) => {
          ...
          if (descendent) {
            ...
            if (this.editor.isShapeOfType(descendent, import_FrameShapeUtil.FrameShapeUtil)) {
              return false;
            }
          }
        });
    ...
    const canShapesDeform = ![...shapeSnapshots.values()].some(
      (shape) => !(0, import_primitives.areAnglesCompatible)(shape.pageRotation, selectionRotation) || shape.isAspectRatioLocked
    );
    return {
      shapeSnapshots,
      selectionBounds,
  ...
  _createShapeSnapshot = (shape) => {
    ...
    return {
      ...
      pageTransform,
      pageRotation: import_primitives.Matrix2d.Decompose(pageTransform).rotation,
...
function rotateSelectionHandle(handle, rotation) {
  rotation = rotation % import_primitives.PI2;
  const numSteps = Math.round(rotation / (import_primitives.PI / 4));
