...
function findAdjacentGaps(gaps, shapeId, gapLength, direction, intersection, depth) {
  ...
  const matches = gaps.filter(
    (gap) => (direction === "forward" ? gap.startNode.id === shapeId : gap.endNode.id === shapeId) && round(gap.length) === round(gapLength) && (0, import_primitives.rangeIntersection)(
    ...
    if (!nextNodes.has(node)) {
      ...
      matches.push(
        ...findAdjacentGaps(
          ...
          gapLength,
          direction,
          (0, import_primitives.rangeIntersection)(
...
class SnapManager {
  ...
  _snapLines = (0, import_signia.atom)("snapLines", void 0);
  get lines() {
    return this._snapLines.value ?? import_signia.EMPTY_ARRAY;
  }
  clear() {
  ...
  get snapPointsCache() {
    return this.editor.store.createComputedCache("snapPoints", (shape) => {
      ...
      return snapPoints.map((point, i) => {
        const { x, y } = import_primitives.Matrix2d.applyToPoint(pageTransfrorm, point);
    ...
    const collectSnappableShapesFromParent = (parentId) => {
      ...
      for (const childId of sortedChildIds) {
        ...
        if (!(pageBounds && renderingBounds.includes(pageBounds)))
          continue;
        if (editor.isShapeOfType(childShape, import_GroupShapeUtil.GroupShapeUtil)) {
    ...
    for (let i = 0; i < sortedShapesHorizontal.length; i++) {
      ...
      for (let j = i + 1; j < sortedShapesHorizontal.length; j++) {
        ...
        if (
          ...
          (0, import_primitives.rangesOverlap)(
          ...
          horizontal.push({
            ...
            endNode,
            startEdge: [
              new import_primitives.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.minY),
              new import_primitives.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.maxY)
            ],
            endEdge: [
              new import_primitives.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.minY),
              new import_primitives.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.maxY)
            ],
            length: endNode.pageBounds.minX - startNode.pageBounds.maxX,
            breadthIntersection: (0, import_primitives.rangeIntersection)(
              startNode.pageBounds.minY,
              startNode.pageBounds.maxY,
              endNode.pageBounds.minY,
              endNode.pageBounds.maxY
      ...
      for (let j = i + 1; j < sortedShapesVertical.length; j++) {
        ...
        if (
          ...
          vertical.push({
            ...
            endNode,
            startEdge: [
              new import_primitives.Vec2d(startNode.pageBounds.minX, startNode.pageBounds.maxY),
              new import_primitives.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.maxY)
            ],
            endEdge: [
              new import_primitives.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.minY),
              new import_primitives.Vec2d(endNode.pageBounds.maxX, endNode.pageBounds.minY)
            ],
  ...
  snapTranslate({
    ...
    const nearestSnapsX = [];
    const nearestSnapsY = [];
    const minOffset = new import_primitives.Vec2d(snapThreshold, snapThreshold);
    ...
    const nudge = new import_primitives.Vec2d(
  ...
  get outlinesInPageSpace() {
    return this.snappableShapes.map(({ id, isClosed }) => {
      const outline = (0, import_utils.deepCopy)(this.editor.getOutlineById(id));
      if (isClosed)
        outline.push(outline[0]);
      const pageTransform = this.editor.getPageTransformById(id);
      if (!pageTransform)
        throw Error("No page transform");
      return import_primitives.Matrix2d.applyToPoints(pageTransform, outline);
    });
  }
  getSnappingHandleDelta({
    ...
    for (const outline of allSegments) {
      for (let i = 0; i < outline.length - 1; i++) {
        C = outline[i];
        D = outline[i + 1];
        nearest = import_primitives.Vec2d.NearestPointOnLineSegment(C, D, handlePoint);
        distance = import_primitives.Vec2d.Dist(handlePoint, nearest);
        if (isNaN(distance))
          continue;
    ...
    if (nearestPoint) {
      ...
      return import_primitives.Vec2d.Sub(nearestPoint, handlePoint);
    }
    return null;
  }
  snapResize({
    ...
    const {
      ...
      scaleX,
      scaleY
    } = import_primitives.Box2d.Resize(
      initialSelectionPageBounds,
      originalHandle,
    ...
    if (scaleX < 0) {
      handle = (0, import_primitives.flipSelectionHandleX)(handle);
    }
    if (scaleY < 0) {
      handle = (0, import_primitives.flipSelectionHandleY)(handle);
    }
    ...
    if (isAspectRatioLocked && (0, import_primitives.isSelectionCorner)(handle) && nudge.len() !== 0) {
    ...
    const snappedDelta = import_primitives.Vec2d.Add(dragDelta, nudge);
    const { box: snappedResizedPageBounds } = import_primitives.Box2d.Resize(
      initialSelectionPageBounds,
      originalHandle,
  ...
  collectPointSnaps({
    ...
    for (const thisSnapPoint of selectionSnapPoints) {
      for (const otherSnapPoint of otherNodeSnapPoints) {
        const offset = import_primitives.Vec2d.Sub(thisSnapPoint, otherSnapPoint);
  ...
  collectGapSnaps({
    ...
    for (const gap of horizontal) {
      if (!(0, import_primitives.rangesOverlap)(
        gap.breadthIntersection[0],
        gap.breadthIntersection[1],
      ...
      if (gapIsLargerThanSelection && round(Math.abs(centerNudge)) <= round(minOffset.x)) {
        ...
        const gapBreadthsOverlap = otherCenterSnap && (0, import_primitives.rangeIntersection)(
          gap.breadthIntersection[0],
          gap.breadthIntersection[1],
    ...
    for (const gap of vertical) {
      ...
      if (gapIsLargerThanSelection && round(Math.abs(centerNudge)) <= round(minOffset.y)) {
        ...
        const gapBreadthsOverlap = otherCenterSnap && (0, import_primitives.rangesOverlap)(
          otherCenterSnap.gap.breadthIntersection[0],
          otherCenterSnap.gap.breadthIntersection[1],
          gap.breadthIntersection[0],
          gap.breadthIntersection[1]
  ...
  getPointSnapLines({
    ...
    return Object.values(snapGroupsX).concat(Object.values(snapGroupsY)).map((snapGroup) => ({
      ...
      points: (0, import_utils.dedupe)(
        snapGroup.map((snap) => import_primitives.Vec2d.From(snap.otherPoint)).concat(snapGroup.map((snap) => import_primitives.Vec2d.From(snap.thisPoint))),
        (a, b) => a.equals(b)
      )
    }));
  }
      ...
      for (const snap of nearestSnapsX) {
        ...
        switch (snap.type) {
          case "gap_center": {
            ...
            const gapBreadthIntersection = (0, import_primitives.rangeIntersection)(
