...
class ClickManager {
  ...
  _getClickTimeout = (state, id = (0, import_data.uniqueId)()) => {
    this._clickId = id;
  ...
  transformPointerDownEvent = (info) => {
    if (!this._clickState)
      return info;
    this._clickScreenPoint = import_primitives.Vec2d.From(info.point);
   ...
   */
  handleMove = () => {
    if (this._clickState !== "idle" && this._clickScreenPoint && this._clickScreenPoint.dist(this.editor.inputs.currentScreenPoint) > (this.editor.isCoarsePointer ? import_constants.COARSE_DRAG_DISTANCE : import_constants.DRAG_DISTANCE)) {
