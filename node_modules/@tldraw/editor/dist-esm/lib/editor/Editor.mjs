...
class Editor extends EventEmitter {
  ...
  get _pageMaskCache() {
    return this.store.createComputedCache("pageMaskCache", (shape) => {
      ...
      ).reduce((acc, b) => b && acc ? intersectPolygonPolygon(acc, b) ?? void 0 : void 0);
      return pageMask;
    });
  }
  /**
      ...
      if (parent) {
        if (!this.viewportPageBounds.includes(this.getPageBounds(parent))) {
          ...
          if (rootShapeIds.length === 1) {
            ...
            if (this.isShapeOfType(parent, FrameShapeUtil) && this.isShapeOfType(rootShape, FrameShapeUtil) && rootShape.props.w === parent?.props.w && rootShape.props.h === parent?.props.h) {
              isDuplicating = true;
            }
          }
        }
...
${JSON.stringify(
  ...
  getOutermostSelectableShape(shape, filter) {
    ...
    let node = shape;
    while (node) {
      if (this.isShapeOfType(node, GroupShapeUtil) && this.focusLayerId !== node.id && !this.hasAncestor(this.focusLayerShape, node.id) && (filter?.(node) ?? true)) {
        match = node;
   ...
   * @public
   */
  animateCamera(x, y, z = this.camera.z, opts = DEFAULT_ANIMATION_OPTIONS) {
    x = Number.isNaN(x) ? 0 : x;
    y = Number.isNaN(y) ? 0 : y;
    z = Number.isNaN(z) ? 1 : z;
