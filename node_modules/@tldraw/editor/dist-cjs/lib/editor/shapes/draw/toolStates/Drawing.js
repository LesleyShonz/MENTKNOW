...
var import_HighlightShapeUtil = require("../../highlight/HighlightShapeUtil");
var import_default_shape_constants = require("../../shared/default-shape-constants");
var import_DrawShapeUtil = require("../DrawShapeUtil");
class Drawing extends import_StateNode.StateNode {
  static id = "drawing";
  info = {};
  initialShape;
  shapeType = this.parent.id === "highlight" ? import_HighlightShapeUtil.HighlightShapeUtil : import_DrawShapeUtil.DrawShapeUtil;
  ...
  onPointerMove = () => {
    ...
    if (this.canDraw) {
      if (inputs.isPen) {
        if (import_primitives.Vec2d.Dist(inputs.currentPagePoint, this.lastRecordedPoint) >= 1 / this.editor.zoomLevel) {
  ...
  getIsClosed(segments, size) {
    ...
    return firstPoint !== lastPoint && this.currentLineLength > strokeWidth * 4 && import_primitives.Vec2d.Dist(firstPoint, lastPoint) < strokeWidth * 2;
  }
  startShape() {
    const {
    ...
    if (this.initialShape) {
      ...
      if (shape && this.segmentMode === "straight") {
        ...
        const prevSegment = (0, import_utils.last)(shape.props.segments);
        if (!prevSegment)
          throw Error("Expected a previous segment!");
        const prevPoint = (0, import_utils.last)(prevSegment.points);
        if (!prevPoint)
        ...
        const prevPointPageSpace = import_primitives.Matrix2d.applyToPoint(
  ...
  updateShapes() {
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        const hasMovedFarEnough = import_primitives.Vec2d.Dist(pagePointWhereNextSegmentChanged, inputs.currentPagePoint) > import_constants.DRAG_DISTANCE;
        if (hasMovedFarEnough) {
          this.pagePointWhereCurrentSegmentChanged = this.pagePointWhereNextSegmentChanged.clone();
          this.pagePointWhereNextSegmentChanged = null;
          this.segmentMode = "straight";
          const prevSegment = (0, import_utils.last)(segments);
          if (!prevSegment)
            throw Error("Expected a previous segment!");
          const prevLastPoint = (0, import_utils.last)(prevSegment.points);
          if (!prevLastPoint)
          ...
          if (prevSegment.type === "straight") {
            this.currentLineLength += import_primitives.Vec2d.Dist(prevLastPoint, newLastPoint);
            newSegment = {
              type: "straight",
              points: [{ ...prevLastPoint }, newLastPoint]
            };
            const transform = this.editor.getPageTransform(shape);
            this.pagePointWhereCurrentSegmentChanged = import_primitives.Matrix2d.applyToPoint(
              transform,
              prevLastPoint
            );
          } else {
      ...
      case "starting_free": {
        ...
        if (hasMovedFarEnough) {
          ...
          const prevPoint = (0, import_utils.last)(prevStraightSegment.points);
          if (!prevPoint) {
          ...
          const newFreeSegment = {
            type: "free",
            points: [
              ...import_primitives.Vec2d.PointsBetween(prevPoint, newPoint, 6).map((p) => p.toFixed().toJson())
            ]
          };
      ...
      case "straight": {
        ...
        if (shouldSnap) {
          if (newSegments.length > 2) {
            ...
            for (let i = 0, n = segments.length - 2; i < n; i++) {
              ...
              const lastPoint = (0, import_utils.last)(segment.points);
              if (!(first && lastPoint))
                continue;
              const nearestPointOnSegment = import_primitives.Vec2d.NearestPointOnLineSegment(
                first,
                lastPoint,
                newPoint2
              );
              const distance = import_primitives.Vec2d.Dist(nearestPointOnSegment, newPoint2);
        ...
        if (didSnap && snapSegment) {
          ...
          const lastPoint = (0, import_utils.last)(snapSegment.points);
          if (!lastPoint)
            throw Error("Expected a last point!");
          const A = import_primitives.Matrix2d.applyToPoint(transform, first);
          const B = import_primitives.Matrix2d.applyToPoint(transform, lastPoint);
          const snappedPoint = import_primitives.Matrix2d.applyToPoint(transform, newPoint2);
          this.editor.snaps.setLines([
            {
              id: (0, import_data.uniqueId)(),
              type: "points",
          ...
          if (shouldSnapToAngle) {
            const currentAngle = import_primitives.Vec2d.Angle(pagePointWhereCurrentSegmentChanged, currentPagePoint);
            const snappedAngle = (0, import_primitives.snapAngle)(currentAngle, 24);
            const angleDiff = snappedAngle - currentAngle;
            pagePoint = import_primitives.Vec2d.RotWith(
              currentPagePoint,
              pagePointWhereCurrentSegmentChanged,
              angleDiff
            );
        ...
        this.currentLineLength += import_primitives.Vec2d.Dist(newSegment.points[0], newPoint2);
      ...
      case "free": {
        ...
        if (newPoints.length && this.mergeNextPoint) {
          ...
          this.currentLineLength += import_primitives.Vec2d.Dist(newPoints[newPoints.length - 1], newPoint);
        ...
        if (newPoints.length > 500) {
          ...
          const newShapeId = (0, import_tlschema.createShapeId)();
          this.editor.createShapes([
            {
              id: newShapeId,
              type: this.shapeType.type,
              x: (0, import_primitives.toFixed)(currentPagePoint.x),
              y: (0, import_primitives.toFixed)(currentPagePoint.y),
              props: {
                isPen: this.isPen,
                segments: [
                  {
          ...
          this.initialShape = (0, import_utils.structuredClone)(this.editor.getShapeById(newShapeId));
          this.mergeNextPoint = false;
          this.lastRecordedPoint = this.editor.inputs.currentPagePoint.clone();
          this.currentLineLength = 0;
        }
  ...
  getLineLength(segments) {
    ...
    for (const segment of segments) {
      for (let i = 0; i < segment.points.length - 1; i++) {
        ...
        length += import_primitives.Vec2d.Sub(B, A).len2();
      }
    }
