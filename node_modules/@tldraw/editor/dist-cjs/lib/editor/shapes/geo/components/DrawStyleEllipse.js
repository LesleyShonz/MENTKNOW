...
const DrawStyleEllipse = React.memo(function DrawStyleEllipse2({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_ShapeFill.ShapeFill, { d: innerPath, color, fill }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: outerPath, fill: `var(--palette-${color})`, strokeWidth: 0, pointerEvents: "all" })
  ] });
});
...
function getEllipseStrokePoints(id, width, height, strokeWidth) {
  const getRandom = (0, import_utils.rng)(id);
  const rx = width / 2;
  const ry = height / 2;
  const perimeter = (0, import_primitives.perimeterOfEllipse)(rx, ry);
  const points = [];
  const start = import_primitives.PI2 * getRandom();
  const length = import_primitives.PI2 + import_primitives.TAU / 2 + Math.abs(getRandom()) * import_primitives.TAU;
  ...
  for (let i = 0; i < count; i++) {
    ...
    points.push(
      new import_primitives.Vec2d(
        ...
        Math.min(
          1,
          0.5 + Math.abs(0.5 - (getRandom() > 0 ? import_primitives.EASINGS.easeInOutSine(t) : import_primitives.EASINGS.easeInExpo(t))) / 2
        )
      )
    );
  }
  return (0, import_primitives.getStrokePoints)(points, getEllipseStrokeOptions(strokeWidth));
}
function getEllipsePath(id, width, height, strokeWidth) {
  const options = getEllipseStrokeOptions(strokeWidth);
  return (0, import_svg.getSvgPathFromStroke)(
    (0, import_primitives.getStrokeOutlinePoints)(
      (0, import_primitives.setStrokePointRadii)(getEllipseStrokePoints(id, width, height, strokeWidth), options),
      options
    )
  );
}
