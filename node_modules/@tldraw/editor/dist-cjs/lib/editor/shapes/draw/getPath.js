...
const PEN_EASING = (t) => t * 0.65 + (0, import_primitives.SIN)(t * import_primitives.PI / 2) * 0.35;
const simulatePressureSettings = (strokeWidth) => {
  return {
    ...
    smoothing: 0.62,
    easing: import_primitives.EASINGS.easeOutSine,
...
const solidSettings = (strokeWidth) => {
  return {
    ...
    smoothing: 0.62,
    simulatePressure: false,
    easing: import_primitives.EASINGS.linear
  };
};
function getHighlightFreehandSettings({
  ...
  return {
    ...
    easing: isPen ? PEN_EASING : import_primitives.EASINGS.easeOutSine,
    last: showAsComplete
  };
}
...
function getPointsFromSegments(segments) {
  ...
  for (const segment of segments) {
    if (segment.type === "free" || segment.points.length < 2) {
      points.push(...segment.points.map(import_primitives.Vec2d.Cast));
    } else {
      const pointsToInterpolate = Math.max(
        4,
        Math.floor(import_primitives.Vec2d.Dist(segment.points[0], segment.points[1]) / 16)
      );
      points.push(...import_primitives.Vec2d.PointsBetween(segment.points[0], segment.points[1], pointsToInterpolate));
    }
  }
  return points;
}
