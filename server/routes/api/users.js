 ...
 * User Management API
 *
 * Provides endpoints for managing users including:
 * - User registration
 * - User authentication
 * - Fetching group members
 * - Fetching the group mentor
 * - Getting the total count of all mentees
 */
...
router.post(
  ...
  [
    ...
    check("accessPin", "Access Pin is required for Mentors").custom(
      (value, { req }) => {
...
async function registerUser(req, res) {
  ...
  var { name, surname, email, password, userType, groupNumber, accessPin } =
    req.body;
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        errors: [{ msg: "Incorrect email address. Please try again" }],
      });
    }
    ...
    if (!isMatch) {
      return res
        .status(400)
        .json({ errors: [{ msg: "Incorrect password. Please try again" }] });
    }
