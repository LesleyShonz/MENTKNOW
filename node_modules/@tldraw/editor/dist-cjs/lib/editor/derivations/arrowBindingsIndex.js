...
const arrowBindingsIndex = (editor) => {
  ...
  return (0, import_signia.computed)("arrowBindingsIndex", (_lastValue, lastComputedEpoch) => {
    if ((0, import_signia.isUninitialized)(_lastValue)) {
      return fromScratch();
    }
    ...
    for (const changes of diff) {
      for (const newShape of Object.values(changes.added)) {
        if (editor.isShapeOfType(newShape, import_ArrowShapeUtil.ArrowShapeUtil)) {
      ...
      for (const [prev, next] of Object.values(changes.updated)) {
        if (!editor.isShapeOfType(prev, import_ArrowShapeUtil.ArrowShapeUtil) || !editor.isShapeOfType(next, import_ArrowShapeUtil.ArrowShapeUtil))
          continue;
      ...
      for (const prev of Object.values(changes.removed)) {
        if (editor.isShapeOfType(prev, import_ArrowShapeUtil.ArrowShapeUtil)) {
