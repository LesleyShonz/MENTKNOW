...
class Cropping extends import_StateNode.StateNode {
  static id = "cropping";
  info = {};
  markId = "";
  snapshot = {};
  ...
  updateShapes() {
    ...
    if (!shape)
      return;
    const util = this.editor.getShapeUtil(import_ImageShapeUtil.ImageShapeUtil);
    if (!util)
      return;
    ...
    const newCrop = (0, import_utils.deepCopy)(crop);
    const newPoint = new import_primitives.Vec2d(shape.x, shape.y);
    const pointDelta = new import_primitives.Vec2d(0, 0);
    ...
    switch (this.info.handle) {
      ...
      case "top_left":
      case "top_right": {
        if (h < import_constants.MIN_CROP_SIZE)
          break;
        hasCropChanged = true;
        newCrop.topLeft.y = newCrop.topLeft.y + change.y / h;
        const heightAfterCrop = h * (newCrop.bottomRight.y - newCrop.topLeft.y);
        if (heightAfterCrop < import_constants.MIN_CROP_SIZE) {
          newCrop.topLeft.y = newCrop.bottomRight.y - import_constants.MIN_CROP_SIZE / h;
      ...
      case "bottom_right": {
        ...
        const heightAfterCrop = h * (newCrop.bottomRight.y - newCrop.topLeft.y);
        if (heightAfterCrop < import_constants.MIN_CROP_SIZE) {
          newCrop.bottomRight.y = newCrop.topLeft.y + import_constants.MIN_CROP_SIZE / h;
        }
        break;
      }
    }
    switch (this.info.handle) {
      ...
      case "top_left":
      case "bottom_left": {
        if (w < import_constants.MIN_CROP_SIZE)
          break;
        hasCropChanged = true;
        newCrop.topLeft.x = newCrop.topLeft.x + change.x / w;
        const widthAfterCrop = w * (newCrop.bottomRight.x - newCrop.topLeft.x);
        if (widthAfterCrop < import_constants.MIN_CROP_SIZE) {
          newCrop.topLeft.x = newCrop.bottomRight.x - import_constants.MIN_CROP_SIZE / w;
      ...
      case "bottom_right": {
        ...
        const widthAfterCrop = w * (newCrop.bottomRight.x - newCrop.topLeft.x);
        if (widthAfterCrop < import_constants.MIN_CROP_SIZE) {
          newCrop.bottomRight.x = newCrop.topLeft.x + import_constants.MIN_CROP_SIZE / w;
        }
        break;
      }
    }
