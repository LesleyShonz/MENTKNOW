...
function getStrokePoints(rawInputPoints, options = {}) {
  ...
  let pts = rawInputPoints.map(import_Vec2d.Vec2d.From);
  ...
  if (pts.length === 0)
    return [
      {
        point: import_Vec2d.Vec2d.From(rawInputPoints[0]),
        input: import_Vec2d.Vec2d.From(rawInputPoints[0]),
        pressure: simulatePressure ? 0.5 : 0.15,
        vector: new import_Vec2d.Vec2d(1, 1),
        distance: 0,
        runningLength: 0,
        radius: 1
      }
  ...
  let pt = pts[1];
  while (pt) {
    if (import_Vec2d.Vec2d.Dist(pt, pts[0]) > size / 3)
      break;
  ...
  pt = pts[pts.length - 1];
  while (pt) {
    if (import_Vec2d.Vec2d.Dist(pt, last) > size / 3)
      break;
    pts.pop();
    pt = pts[pts.length - 1];
  ...
  pts.push(last);
  const isComplete = options.last || !options.simulatePressure || pts.length > 1 && import_Vec2d.Vec2d.Dist(pts[pts.length - 1], pts[pts.length - 2]) < size || pointsRemovedFromNearEnd > 0;
  if (pts.length === 2 && options.simulatePressure) {
    const last2 = pts[1];
    pts = pts.slice(0, -1);
    for (let i = 1; i < 5; i++) {
      const next = import_Vec2d.Vec2d.Lrp(pts[0], last2, i / 4);
  ...
  for (let i = 1, n = pts.length; i < n; i++) {
    ...
    if (prev.point.equals(point))
      continue;
    distance = import_Vec2d.Vec2d.Dist(point, prev.point);
    ...
    prev = {
      ...
      vector: import_Vec2d.Vec2d.Sub(prev.point, point).uni(),
