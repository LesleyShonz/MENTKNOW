...
class MinimapManager {
  ...
  pageBounds = [];
  collaborators = [];
  canvasScreenBounds = new import_primitives.Box2d();
  canvasPageBounds = new import_primitives.Box2d();
  contentPageBounds = new import_primitives.Box2d();
  contentScreenBounds = new import_primitives.Box2d();
  originPagePoint = new import_primitives.Vec2d();
  originPageCenter = new import_primitives.Vec2d();
  isInViewport = false;
  debug = false;
  setDpr(dpr) {
  ...
  minimapScreenPointToPagePoint = (x, y, shiftKey = false, clampToBounds = false) => {
    ...
    if (clampToBounds) {
      ...
      px = (0, import_primitives.clamp)(px, minX, maxX);
      py = (0, import_primitives.clamp)(py, minY, maxY);
    }
    if (shiftKey) {
  ...
  render = () => {
    ...
    for (let i = 0, n = pageBounds.length; i < n; i++) {
      ...
      MinimapManager.roundedRect(
        ...
        pb.width,
        pb.height,
        (0, import_primitives.clamp)(rx, ax, pb.width / bx),
        (0, import_primitives.clamp)(ry, ay, pb.height / by)
      );
    }
    ...
    if (this.debug) {
      const commonBounds = import_primitives.Box2d.Common(pageBounds);
    ...
    for (let i = 0; i < this.collaborators.length; i++) {
      ...
      ctx.beginPath();
      ctx.ellipse(
        (0, import_primitives.clamp)(collaborator.cursor.x, canvasPageBounds.minX + px, canvasPageBounds.maxX - px),
        (0, import_primitives.clamp)(collaborator.cursor.y, canvasPageBounds.minY + py, canvasPageBounds.maxY - py),
        5 / sx,
        5 / sy,
        0,
        0,
        import_primitives.PI2
      );
