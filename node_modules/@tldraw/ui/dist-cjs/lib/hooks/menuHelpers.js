...
const useThreeStackableItems = () => {
  ...
  return (0, import_signia_react.useValue)("threeStackableItems", () => shapesWithUnboundArrows(editor).length > 2, [editor]);
};
const useAllowGroup = () => {
  const editor = (0, import_editor.useEditor)();
  return (0, import_signia_react.useValue)("allowGroup", () => shapesWithUnboundArrows(editor).length > 1, [editor]);
};
...
function findMenuItem(menu, path) {
  ...
  (0, import_utils.assert)(item, `Menu item ${path.join(" > ")} not found`);
  return item;
}
function _findMenuItem(menu, path) {
  ...
  switch (item.type) {
    ...
    default:
      (0, import_utils.exhaustiveSwitchError)(item, "type");
  }
}
