...
var BaseSegment2d_exports = {};
__export(BaseSegment2d_exports, {
  BaseSegment2d: () => BaseSegment2d
});
module.exports = __toCommonJS(BaseSegment2d_exports);
...
class BaseSegment2d {
  ...
  get bounds() {
    ...
    const result = new import_Box2d.Box2d(minX, minY, maxX - minX, maxY - minY);
  ...
  getClosestPointTo(point) {
    ...
    for (let i = 1; i < lut.length; i++) {
      const distance = import_Vec2d.Vec2d.Dist(lut[i], point);
    ...
    for (let t = Math.max(0, closestT - step / 2); t < Math.min(1, closestT + step / 2); t += step / 10) {
      const pt = this.getPoint(t);
      const distance = import_Vec2d.Vec2d.Dist(pt, point);
      if (distance < closestDistance) {
        closestDistance = distance;
        closestPoint = pt;
      }
...
//# sourceMappingURL=BaseSegment2d.js.map
